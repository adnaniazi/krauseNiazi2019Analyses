...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "krauseNiazi2019Analyses", "tests")); system.time(test_check("krauseNiazi2019Analyses", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-export _R_CHECK_CRAN_INCOMING_REMOTE_=FALSE && R CMD check --as-cran krauseNiazi2019Analyses_$(VERSION).tar.gz
	rm -rf krauseNiazi2019Analyses.Rcheck/
