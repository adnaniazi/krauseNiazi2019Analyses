...
if (save_figures) {
  ggsave(plot = p,  
         ...
         bg = "white", width = 5, height = 3, units = 'in')
}
```
...
if (save_figures) {
  ggsave(plot = p,  
         ...
         bg = "white", width = 10, height = 4, units = 'in')
}
```
...
if (save_figures) {
  ggsave(plot = p,  
         ...
         bg = "white", width = 4, height = 4, units = 'in')
}
```
...
# ISMB POSTER RNA Vs. DNA Translocation rate comparison
We first load RNA data
```{r }
loadd(rna_kr_data)
```
...
```{r }
polyat_dna <- select(dna_kr_data, samples_per_nt_st, barcode) %>% 
  mutate(barcode = as.numeric(as.character(barcode)))

polya_rna <-select(rna_kr_data, samples_per_nt_tf, barcode) %>% 
  mutate(barcode = as.numeric(as.character(barcode)))
...
# combine rna and dna datasets
```{r}
polya_rna_subset %<>% rename(samples_per_nt = samples_per_nt_tf) %>% 
  mutate(data_type = 'RNA')

polyat_dna %<>% mutate(data_type = 'DNA') %>% 
  rename(samples_per_nt = samples_per_nt_st)
...
```{r, warning=FALSE, message=FALSE}
p <- ggplot(data = df, aes(x = data_type, y = samples_per_nt, 
                           color = data_type, fill = data_type)) +
  geom_two_sided_flat_violin(alpha = .5) 
```
...
p <- p + theme(panel.background = element_blank(),
  ...
  scale_color_brewer(palette = "Paired", name = lengend_name, labels = legend_labels) +
  scale_fill_brewer(palette = "Paired", name = lengend_name, labels = legend_labels) +
  scale_y_continuous(minor_breaks = seq(0, 50, 10)) +
...
```{r, warning=FALSE, echo=FALSE, message=FALSE, results="hide"}
if (save_figures) {
  RSvgDevice::devSVG(here('figures', 'dna_kr_tf_samples_per_nt_dna_vs_rna_density_plot.svg'), 
                     width = 5.7, height = 3.5, onefile = TRUE, xmlHeader = TRUE)
  print(p)
  dev.off()
}
